...
def imshow_plt(input_image: np.ndarray, title = "Image", dpi=600):
...
def imshow_cv(input_image: np.ndarray,title = "", cvt_2_bgr: bool=False):
    "Plot Image using opencv"
    if not title:
        title = "Image, press 0 to exit the screen..."
    if cvt_2_bgr:
...
def imflow(input_image: np.ndarray,title = "", cvt_2_bgr: bool=False, duration=1):
    """visualize sequence of images as in a video, 
    call this method each time on a frame, 
    it will reload the new image in the same frame and show for a given duration"""
